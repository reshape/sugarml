...
module.exports = function Lexer(input, options = {}) {
  ...
  function tag() {
    if (char.match(/\w/)) {
      ...
      //Prev tag is inline if we got a newTagContext
      isPrevTagInline = newTagContext;

      //Reset inline tag context
      newTagContext = false
  ...
  function classId() {
    ...
    if (char === '#' || `${char}${nextChar()}`.match(/^\.\S$/)) {
      ...
      if (
        !lastToken ||
        ['tag', 'attributeKey', 'attributeValue'].indexOf(lastToken.type) < 0 ||
        newTagContext
      ) {
        addToken('tag', 'div')
        newTagContext = false
  ...
  function indentOrTagContent() {
    if (char && char.match(/\s/) && !char.match(/\n/)) {
      ...
      if (lastToken.type === 'newline') {
        while (char && char.match(/\s/) && !char.match(/\n/)) {
          indent += char
          indentLevel++
          next()
        }
        if (isPrevTagInline && indentLevel > previousIndent){
          throw new SugarmlError({
            message: `Indent level different from previous line : forbidden after nested tag`,
            location: {
              filename: options.filename,
              line: line,
              col: col,
  ...
  function newline() {
    if (char && char.match(/\n/)) {
      ...
      next()
      line++
      col = 0 // increment the line number, reset the col
    }
  }
  ...
  function comment() {
    ...
    if (char === '/' && nextChar() === '/') {
      ...
      if (char.match(/\s/)) space = true

      // grab the comment body
      let comment = collectUntil('\n')
      if (space) comment += ' '
  ...
  function contentBlockCollect(minIndent) {
    ...
    const baseIndent = Math.min.apply(
      Math,
      lines.map(l => {
