...
module.exports = function Lexer (input, options) {
    ...
    if (char === '(') {
      ...
      while (char !== ')') {
        ...
        if (char === '=') {
          ...
          let regex = /[\s\)'"]/
          if (quoted) { regex = new RegExp(quoted) }
  ...
  function blockContent () {
    if (`${char}${nextChar()}`.match(/^\.\s$/)) {
      ...
      if (match.index === 0) {
        ...
        // last token is the tag, one before is indent if applicable
        const maybeIndent = tokens[tokens.length - 2]
  ...
  function lookFor (re) {
    return re.exec(input.substring(current, input.length))
  }

  function collectUntil (re) {
    let content = ''
    while (char && !char.match(re)) {
      content += char
      next()
    }
    return content
  ...
  function contentBlockCollect (minIndent) {
    let content = _contentBlockCollect(minIndent)

    // calculate the minimum indentation across all lines of content
    const lines = content.split('\n')
    const baseIndent = Math.min.apply(Math, lines.map((l) => l.match(/[^\S\n]*/)[0].length))

    // now remove the min indent from each line and return
    content = lines.map((l) => l.substring(baseIndent)).join('\n')
    return content
  }
  ...
  function _contentBlockCollect (minIndent) {
    ...
    const match2 = lookFor(/[^\S\n]*/)
    if (match2 && match2[0].length > minIndent) {
      // move past the base indent level
      next(minIndent)
